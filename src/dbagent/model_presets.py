...
inside an in-memory :class:`DBAgentConfig`.
"""
...
def resolve_model_selection(
    selection: str,
    *,
    active_config: DBAgentConfig,
    startup_config: DBAgentConfig,
) -> DBAgentConfig:
...
def default_deepseek_key_file() -> Path:
    ...
    configured = os.environ.get("DBAGENT_DEEPSEEK_KEY_FILE", "").strip()
    if configured:
...
def load_deepseek_api_key(path: Path | None = None) -> str:
    ...
    environment_key = os.environ.get("DBAGENT_DEEPSEEK_API_KEY", "").strip()
    if environment_key:
    ...
    try:
        ...
        raise ConfigurationError(
            f"DeepSeek key is not available. Set DBAGENT_DEEPSEEK_API_KEY or create "
