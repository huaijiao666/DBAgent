...
from dbagent.trace import sanitize_for_storage
...
class SessionStore:
    ...
    ``.dbagent/session.json`` format remains readable as the special ``legacy``
    session so upgrading DBA never silently discards the user's latest history.
    """

    def __init__(self, workspace: Path) -> None:
        dbagent_directory = workspace.resolve() / ".dbagent"
        self.sessions_directory = dbagent_directory / "sessions"
        self.legacy_path = dbagent_directory / "session.json"
